...
public class DirectedSinglyLinkedListGraph implements Graph {
    ...
    public void removeVertexEdge(int vertex) throws ListException, GraphException {
        if(containsVertex(new Vertex(vertex))) {
            for (int i = 0; i < size(); i++) {
                if (containsEdge(vertex, i)) {
                    removeEdge(vertex, i);

                }
                if (containsEdge(i, vertex)) {
                    removeEdge(i, vertex);
                }
            removeVertex(vertex);
            }
